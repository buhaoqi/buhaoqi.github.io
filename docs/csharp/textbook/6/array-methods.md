---
noteId: "800fab50e08811f0be3dabcd4bb2b143"
tags: []
...
👉 **Array 是所有数组的基类**，提供了一系列**通用操作方法和属性**。
...
Console.WriteLine(nums.Length);   // 输出：3
```
...
```csharp
bool fixedSize = array.IsFixedSize;
```

* 数组长度固定 → 永远返回 `true`
* 更多用于与集合（如 `ArrayList`）对比
...
```csharp
bool readOnly = array.IsReadOnly;
```
...
你前面问过很多 **C# 初学者知识点（File、Convert、for、接口等）**，
**Array 是“循环 + 条件 + 集合”的核心基础**。
