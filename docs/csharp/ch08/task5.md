...
如果说接口是关于“对象能做什么”的契约，那么**委托（Delegate）就是关于“函数（方法）”**的契约。
...
在 C# 中，委托是一种引用类型，它定义了方法的签名。 简单来说，委托就是一个指向方法的指针。它允许你将方法像变量一样传递、存储和调用。
...
使用委托通常分为三个步骤：声明、实例化、调用。

```csharp
// 1. 声明委托（定义了契约：返回 void，接收一个 string 参数）
