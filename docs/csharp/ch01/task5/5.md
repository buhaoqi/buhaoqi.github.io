...
## 二、凯撒密码（移位加密）
### 1.  算法简介
凯撒密码是最基础的「移位加密算法」，核心是将字符串中的每个字母按照**固定步长（偏移量）**在字母表中移动，得到加密后的字符。
...
## 算法3： 异或加密（简单位运算加密）—— 编程考点、基础位运算应用
### 1.  算法简介
异或加密是基于「二进制位运算（异或^）」的简单加密算法，核心利用异或的**两个关键特性**（中职必须熟记），是编程题中的常见考点。
- 异或特性1：`a ^ a = 0`（任何数与自身异或，结果为0）；
- 异或特性2：`a ^ b ^ b = a`（任何数与同一个数异或两次，结果还原为自身）；
- 特点：加密和解密使用同一个「密钥（固定数字）」，代码简洁，可逆性强。

### 2.  核心原理（中职重点掌握）
1.  核心流程：`字符ASCII码 ^ 密钥 → 加密后ASCII码 → 还原字符`；
2.  加密步骤：遍历字符串，将每个字符的ASCII码与「固定密钥（如123）」进行异或运算，得到加密后的字符；
3.  解密步骤：将加密后的字符ASCII码与「同一个密钥」再次异或，即可还原原字符（利用特性2）。

### 3.  手动加密示例（了解即可，编程题重点）
**题目**：以密钥`key=10`，加密字符`'a'`（ASCII=97）。
- 步骤拆解：
  1.  转换二进制：`97` → `0110 0001`，`10` → `0000 1010`；
  2.  异或运算：`0110 0001 ^ 0000 1010 = 0110 1011`；
  3.  还原十进制：`0110 1011` → `107` → 字符`'k'`；
- 解密：`107 ^ 10 = 97` → `'a'`（还原成功）。
...
class XorCipher
{
    static void Main(string[] args)
    {
        string originalStr = "hello123";
        // 密钥（中职考试中通常给出固定正整数，如10、123）
        int key = 10;

        // 加密
        string encryptedStr = XorEncrypt(originalStr, key);
        Console.WriteLine($"加密前：{originalStr}");
        Console.WriteLine($"加密后：{encryptedStr}");

        // 解密（使用同一个密钥）
        string decryptedStr = XorEncrypt(encryptedStr, key);
        Console.WriteLine($"解密后：{decryptedStr}");
    }

    /// <summary>
    /// 异或加密/解密（同一个方法，密钥相同即可）
    /// </summary>
    static string XorEncrypt(string str, int key)
    {
        string result = "";
        foreach (char c in str)
        {
            // 核心：字符ASCII码 与 密钥 进行异或运算
            char processedChar = (char)(c ^ key);
...
解密后：hello123
```
> 说明：加密后的字符可能是不可见字符（如``），这是正常现象，不影响解密还原。
...
3.  **编程重点**：异或加密（利用异或两大特性）、凯撒密码的代码实现，重点掌握`foreach`遍历字符串、字符与ASCII码的转换、循环移位公式。
4.  **关键技巧**：所有基础算法都是「可逆的」，加密与解密是逆过程，熟记每种算法的加密/解密公式，避免混淆。
5.  **注意事项**：非字母/数字字符（如空格、符号）通常直接保留，无需参与加密变换，这是笔试和编程题的常见要求。